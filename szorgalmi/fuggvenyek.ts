...
//Üres tömb feltöltő
function TombFeltolto(tombHossz:number){
    var uresTomb: number[] = []; 
...
function LegnagyobbElem(vizsgalandoTomb:number[]):number{
...
function LegkisebbElem(vizsgalandoTomb:number[]):number{
...
// A tömb összege:
function TombOsszege(vizsgalandoTomb:number[]):number{
    var osszeg:number = 0;
...
// A tömb átlaga:
function TombAtlaga(vizsgalandoTomb:number[]):number{
    var osszeg:number = 0;
    for (var i=0; i<vizsgalandoTomb.length; i++){
       osszeg+=vizsgalandoTomb[i]; 
    }
...
function TombMetszet(tomb1:number[],tomb2:number[]):any{
    var ujTomb:number[]=[]
    ...
    var uzenet:string ="Nincs metszete a két tömbnek" 
    if(ujTomb.length>=1){
        return ujTomb;
    }
    else{
...
function AdottEviCsatlakozok(ev:string){
    var orszagTomb:string[]=[];
    ...
    var szoveg:string="Nincs adott évben csatlakozás"
...
//orszag keresés
function OrszagKereso(keresettOrszag:string):string{
    for(var i=0; i<EuropaiUnio.length; i++){
        var szerepelE:boolean=false;
        if(EuropaiUnio[i].orszag==keresettOrszag){
            szerepelE=true;
            var szoveg1:string="A keresett ország szerepel a listában";
            return szoveg1;
        }
    }
    if(szerepelE==false){
        var szoveg2:string="A keresett ország NEM szerepel a listában";
        return szoveg2;
    }
